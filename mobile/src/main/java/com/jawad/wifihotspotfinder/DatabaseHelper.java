...
// required by the user, such as the user's home and work locations.
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // Opens the database that is in the directory as a read and writable database.
    ...
    // These two methods below must be declared as they are the extended
    // part of the SQLiteOpenHelper without which the DatabaseHelper class
    // cannot perform the same functions as SQLiteOpenHelper.

    // As we are not creating new databases but rather copying them
    // from the database folder, the onCreate method is left empty.
    @Override
    public void onCreate(SQLiteDatabase db) {
    }

    // As we the code doesn't require any anything to be done after
    // the database is updated, the onUpgrade method is left empty.
    @Override
